...
function seletor($genero, $sqlconn)
{
    ...
    foreach ($reg as $filme) {
        ...
        $query_likes = $sqlconn->prepare("SELECT COUNT(*) as total_deslikes FROM interacoes WHERE id_conteudo = ? AND avaliacoes = 'Deslike'");
        $query_likes->execute([$filme['id_conteudo']]);
        $deslike_count = $query_likes->fetch(PDO::FETCH_ASSOC)['total_deslikes'];
